...
function resolveRoutePath(): string
{
    ...
    $requestPath = rawurldecode(trim((string) $requestPath));

    $scriptDir = str_replace('\\', '/', dirname((string) ($_SERVER['SCRIPT_NAME'] ?? '')));
    $scriptDir = rawurldecode($scriptDir);
...
if (!empty($segments)) {
    ...
    if ($first === 'admin' && strtolower($second) === 'login') {
        ...
        $action = 'adminLogin';
    } elseif (in_array($first, ['login', 'register', 'signup', 'logout', 'authenticate'], true) && strtolower($second) === 'login') {
        $controller = 'AuthController';
        $action = 'login';
