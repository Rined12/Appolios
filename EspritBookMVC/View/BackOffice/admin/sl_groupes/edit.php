...
$formData = [
    ...
    'approval_statut' => $old['approval_statut'] ?? ($groupe['approval_statut'] ?? 'en_cours'),
];
    ...
    <div class="container admin-dashboard-container">
        <div class="admin-layout">
            ...
            <div class="admin-main">
                <div class="dashboard-header"><h1>Modifier le groupe</h1></div>
                <div class="form-container" style="max-width:700px;">
                    <form method="POST" action="<?= APP_ENTRY ?>?url=admin/sl-groupes/<?= (int) $groupe['id_groupe'] ?>/update" enctype="multipart/form-data" novalidate id="admin-groupe-edit-form">
                        ...
                        <?php if ($currentCover !== ''): ?>
                        <div class="form-group">
                            <span style="display:block;margin-bottom:6px;font-weight:600;">Photo actuelle</span>
                            <img src="<?= htmlspecialchars($currentCover) ?>" alt="" style="max-width:100%;max-height:180px;border-radius:10px;border:1px solid #e2e8f0;">
                        </div>
                        <?php endif; ?>
                        <div class="form-group">
                            <label for="group_photo"><?= $currentCover !== '' ? 'Remplacer la photo' : 'Photo du groupe (optionnel)' ?></label>
                            <input type="file" id="group_photo" name="group_photo">
                            <p style="margin:6px 0 0;font-size:13px;color:#64748b;">JPEG, PNG, GIF ou WebP, 2 Mo maximum. Laisser vide pour conserver l image.</p>
                        ...
                        <div class="form-group">
                            ...
                            <select id="statut" name="statut">
                                ...
                                <option value="archivé" <?= ($formData['statut'] === 'archivé') ? 'selected' : '' ?>>Archive</option>
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select id="approval_statut" name="approval_statut">
                                <option value="en_cours" <?= ($formData['approval_statut'] === 'en_cours') ? 'selected' : '' ?>>En cours (validation)</option>
                                <option value="en_attente" <?= ($formData['approval_statut'] === 'en_attente') ? 'selected' : '' ?>>En attente (legacy)</option>
                                <option value="approuve" <?= ($formData['approval_statut'] === 'approuve') ? 'selected' : '' ?>>Approuve</option>
                                <option value="rejete" <?= ($formData['approval_statut'] === 'rejete') ? 'selected' : '' ?>>Rejete</option>
                            </select>
                            <div class="field-error" data-error-for="approval_statut" style="color:#ef4444;"><?= htmlspecialchars($errors['approval_statut'] ?? '') ?></div>
                        </div>
                        <button class="btn btn-yellow" type="submit">Mettre a jour</button>
                    </form>
                </div>
            </div>
        </div>
...
document.addEventListener('DOMContentLoaded', function () {
    const form = document.getElementById('admin-groupe-edit-form');
