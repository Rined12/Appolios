...
class Groupe extends BaseModel
{
    ...
    /**
     * Ensures an approval column exists (VARCHAR) so student/admin workflows and filters work.
     */
    private function ensureApprovalSchema(): void
    {
        ...
        try {
            ...
            // Permission denied or duplicate column
        }
    }
    ...
    /**
     * Ensures a VARCHAR column exists for the group cover URL (MySQL).
     */
    private function ensureImageSchema(): void
    {
        ...
        try {
            ...
            // Permission denied or table mismatch — ignore
        }
    }
    ...
    public function countApproved(): int
    {
        ...
        $stmt = $this->db->query("SELECT COUNT(*) AS total FROM groupe WHERE {$approvalCol} = 'approuve'");
        $row = $stmt->fetch();
    ...
    public function getRoleUser($id_groupe, $id_user): ?string
    {
        $stmt = $this->db->prepare("SELECT role FROM groupe_user WHERE id_groupe = ? AND id_user = ?");
        $stmt->execute([(int) $id_groupe, (int) $id_user]);
