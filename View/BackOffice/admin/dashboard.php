...
<div style="background: white; border-radius: 16px; padding: 1.5rem; box-shadow: 0 4px 15px rgba(0,0,0,0.02); border: 1px solid #eef2f6; display: flex; align-items: center; gap: 1.2rem; transition: transform 0.2s;" onmouseover="this.style.transform='translateY(-3px)'" onmouseout="this.style.transform='translateY(0)'">
                    ...
                    <a href="<?= APP_ENTRY ?>?url=admin/course-requests" style="text-decoration: none; background: white; border-radius: 16px; padding: 1.5rem; box-shadow: 0 4px 15px rgba(0,0,0,0.02); border: 1px solid #eef2f6; display: flex; align-items: center; gap: 1.2rem; transition: transform 0.2s;" onmouseover="this.style.transform='translateY(-3px)'" onmouseout="this.style.transform='translateY(0)'">
                        <div style="width: 54px; height: 54px; border-radius: 14px; background: #f0fdf4; color: #22c55e; display: flex; align-items: center; justify-content: center; flex-shrink: 0;">
                        ...
                        <div>
                            <h3 style="margin: 0 0 0.2rem 0; font-size: 1.6rem; font-weight: 800; color: #1e293b; line-height: 1;"><?= $pendingCourses ?? 0 ?></h3>
                            <p style="margin: 0; font-size: 0.9rem; color: #64748b; font-weight: 600;">Course Requests</p>
                        </div>
                        <?php if (($pendingCourses ?? 0) > 0): ?>
...
(function() {
    ...
    var monthlyData = <?php echo json_encode($monthlyCourseStats ?? []); ?>;

    // Status Distribution Chart (Doughnut)
    var statusCtx = document.getElementById('statusChart');
    if (statusCtx) {
        var labels = statusData.map(function(item) { return item.status; });
        var counts = statusData.map(function(item) { return parseInt(item.count); });
        new Chart(statusCtx, {
            type: 'doughnut',
            data: {
                labels: labels,
    ...
    // Monthly Course Additions Chart (Line)
