...
foreach (($eventStats ?? []) as $stat) {
    ...
    $eventLabels[]    = $title;
    $capaciteData[]   = (int)($stat['capacite_max']      ?? 0);
    $participantData[] = (int)($stat['participant_count'] ?? 0);
}

$typeLabels = [];
$typeData   = [];
foreach (($typeStats ?? []) as $ts) {
    $typeLabels[] = ucfirst($ts['type'] ?? 'Autre');
...
foreach (($participantTypeStats ?? []) as $pts) {
    $partTypeLabels[] = ucfirst($pts['type'] ?? 'Autre');
    $partTypeData[]   = (int)$pts['participant_count'];
}

$totalE = count($eventStats ?? []);
$totalP = array_sum($participantData);

// Status map
$statusMap = ['planifie' => 0, 'en_cours' => 0, 'termine' => 0, 'annule' => 0];
foreach (($statusStats ?? []) as $ss) {
    $statusMap[$ss['statut']] = (int)$ss['count'];
}
?>
...
<style>
    .stat-card {
        ...
        overflow: hidden;    /* Prevent chart bleed */
    ...
    <div class="neo-glass-card" style="width: 100%; background: #0f172a; border-radius: 20px; box-shadow: 0 15px 40px rgba(0, 0, 0, 0.2); border: 1px solid rgba(255, 255, 255, 0.1); overflow: hidden; display: flex; flex-direction: column;">
        ...
        <div style="padding: 3rem 3.5rem; background: transparent;">
            ...
            <div class="stat-grid-2" style="margin-bottom: 2.5rem;">
                <!-- Bar Chart: Events vs Participations -->
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    // 1. Bar Chart (Events vs Participation)
    ...
    // 3. Participations per Event Type (Polar Area)
