...
<section class="neo-auth-wrap neo-login-page" style="padding: 2rem; background: #020617; min-height: 100vh; display: flex; align-items: center; justify-content: center; font-family: 'Inter', sans-serif;">
    <div class="neo-glass-card neo-auth-grid" style="max-width: 1150px; width: 100%; background: #0f172a; border-radius: 20px; box-shadow: 0 15px 40px rgba(0, 0, 0, 0.2); border: 1px solid rgba(255, 255, 255, 0.1); overflow: hidden; display: grid; grid-template-columns: 1fr 1.1fr; gap: 0;">
        
        <!-- Left Side: Visual & Info -->
        <aside class="neo-auth-info" style="background: rgba(30, 41, 59, 0.3); padding: 3.5rem 3rem; display: flex; flex-direction: column; justify-content: space-between; border-right: 1px solid rgba(255, 255, 255, 0.05); position: relative; overflow: hidden;">
            <!-- Decorative blobs -->
            ...
            <div style="position: relative; z-index: 2;">
                ...
                <a href="<?= APP_ENTRY ?>?url=event/evenements" style="display: inline-flex; align-items: center; gap: 8px; font-size: 0.9rem; color: #cbd5e1; font-weight: 600; text-decoration: none; margin-bottom: 2rem; transition: color 0.2s;" onmouseover="this.style.color='#E19864'" onmouseout="this.style.color='#cbd5e1'">
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><line x1="19" y1="12" x2="5" y2="12"></line><polyline points="12 19 5 12 12 5"></polyline></svg>
                    Back to Evenements
                </a>

                <h2 style="font-size: 2.5rem; font-weight: 800; color: #ffffff; line-height: 1.15; margin: 0 0 1rem 0; letter-spacing: -0.02em;">
                    Teacher<br><span style="color: #E19864;">Requests</span>
                </h2>
                <p style="color: #94a3b8; font-size: 1.05rem; line-height: 1.6; margin: 0 0 2rem 0; max-width: 90%;">
                ...
                <div class="neo-badges" style="display: flex; gap: 10px; flex-wrap: wrap;">
                    <span style="background: rgba(255, 255, 255, 0.05); color: #cbd5e1; border: 1px solid rgba(255, 255, 255, 0.1); padding: 6px 14px; border-radius: 50px; font-size: 0.8rem; font-weight: 700; letter-spacing: 0.05em; text-transform: uppercase;">Pending Review</span>
        ...
        <div class="neo-auth-form" style="padding: 3.5rem 3rem; background: transparent; display: flex; flex-direction: column;">
            <div style="display: flex; justify-content: space-between; align-items: flex-end; margin-bottom: 2rem;">
                <div>
                    <h2 style="font-size: 1.8rem; font-weight: 700; color: #ffffff; margin: 0 0 0.4rem 0;">Action Required</h2>
                    <p style="color: #94a3b8; font-size: 0.95rem; margin: 0;">Approve or reject the following submissions.</p>
                </div>
                <div style="background: rgba(225, 152, 100, 0.15); color: #E19864; font-weight: 700; padding: 6px 12px; border-radius: 8px; font-size: 0.9rem;">
            ...
            <div style="overflow-y: auto; padding-right: 10px; flex-grow: 1; max-height: 500px; display: flex; flex-direction: column; gap: 1.25rem;">
                <?php if (!empty($requests)): ?>
                    <?php foreach ($requests as $event): ?>
                        <div style="background: rgba(30, 41, 59, 0.5); border: 1.5px solid rgba(255, 255, 255, 0.05); border-radius: 14px; padding: 1.5rem; transition: all 0.2s ease; box-shadow: 0 4px 15px rgba(0,0,0,0.1);" onmouseover="this.style.borderColor='rgba(225, 152, 100, 0.3)'; this.style.boxShadow='0 8px 25px rgba(0,0,0,0.2)'; this.style.transform='translateY(-2px)'" onmouseout="this.style.borderColor='rgba(255, 255, 255, 0.05)'; this.style.boxShadow='0 4px 15px rgba(0,0,0,0.1)'; this.style.transform='translateY(0)'">
                            
                            <!-- Header: Teacher info -->
                            <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 1.2rem;">
                                <div style="display: flex; align-items: center; gap: 12px;">
                                    <div style="width: 44px; height: 44px; background: linear-gradient(135deg, #E19864 0%, #d9804b 100%); color: #fff; border-radius: 10px; display: flex; align-items: center; justify-content: center; font-weight: 700; font-size: 1.2rem; box-shadow: 0 4px 10px rgba(225, 152, 100, 0.2);">
                                        <?= strtoupper(substr($event['creator_name'] ?? 'T', 0, 1)) ?>
                                    </div>
                                    <div>
                                        <h4 style="margin: 0 0 2px 0; color: #ffffff; font-size: 1.05rem; font-weight: 700;"><?= htmlspecialchars($event['creator_name'] ?? 'Teacher') ?></h4>
                            ...
                            <h3 style="margin: 0 0 0.75rem 0; color: #ffffff; font-size: 1.2rem; font-weight: 600; line-height: 1.3;">
                                <?= htmlspecialchars(($event['titre'] ?? '') ?: ($event['title'] ?? '')) ?>
                            </h3>
                            
                            <div style="display: flex; gap: 1rem; margin-bottom: 1.5rem; flex-wrap: wrap; background: rgba(30, 41, 59, 0.8); padding: 10px; border-radius: 8px;">
                                <div style="display: flex; align-items: center; gap: 6px; color: #cbd5e1; font-size: 0.85rem; font-weight: 500;">
                                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#E19864" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"></rect><line x1="16" y1="2" x2="16" y2="6"></line><line x1="8" y1="2" x2="8" y2="6"></line><line x1="3" y1="10" x2="21" y2="10"></line></svg>
                            ...
                            <div style="display: flex; gap: 10px; align-items: stretch; flex-wrap: wrap;">
                                ...
                                <button type="button"
                                    ...
                                    style="background: rgba(255, 255, 255, 0.05); border: 1.5px solid rgba(255, 255, 255, 0.1); color: #cbd5e1; height: 42px; padding: 0 1.2rem; border-radius: 8px; font-weight: 600; font-size: 0.9rem; cursor: pointer; display: flex; align-items: center; gap: 6px; transition: all 0.2s;"
                                    onmouseover="this.style.background='rgba(255, 255, 255, 0.1)'; this.style.borderColor='#E19864'; this.style.color='#E19864';"
                                ...
                                <form action="<?= APP_ENTRY ?>?url=event/approve-evenement/<?= (int) $event['id'] ?>" method="POST" style="flex: 0 0 auto;">
                                    <button type="submit" style="background: linear-gradient(135deg, #E19864 0%, #d9804b 100%); color: #fff; border: none; padding: 0 1.2rem; height: 42px; border-radius: 8px; font-weight: 600; font-size: 0.9rem; cursor: pointer; display: flex; align-items: center; justify-content: center; transition: all 0.2s ease; box-shadow: 0 4px 10px rgba(225, 152, 100, 0.2);" onmouseover="this.style.boxShadow='0 6px 15px rgba(225, 152, 100, 0.3)'; this.style.transform='translateY(-1px)'" onmouseout="this.style.boxShadow='0 4px 10px rgba(225, 152, 100, 0.2)'; this.style.transform='translateY(0)'">
                                ...
                                <form action="<?= APP_ENTRY ?>?url=event/reject-evenement/<?= (int) $event['id'] ?>" method="POST" style="flex: 1; display: flex; flex-direction: column; gap: 8px;" novalidate>
                                    <div style="display: flex; gap: 8px; width: 100%;">
                                        <input type="text" name="rejection_reason" placeholder="Rejection reason..." style="flex: 1; background: <?= isset($errors['rejection_reason_' . $event['id']]) ? 'rgba(239, 68, 68, 0.1)' : 'rgba(30, 41, 59, 0.5)' ?>; border: 1.5px solid <?= isset($errors['rejection_reason_' . $event['id']]) ? '#ef4444' : 'rgba(255, 255, 255, 0.1)' ?>; border-radius: 8px; padding: 0 1rem; height: 42px; color: #ffffff; font-size: 0.9rem; outline: none; transition: border-color 0.2s;" onfocus="this.style.borderColor='<?= isset($errors['rejection_reason_' . $event['id']]) ? '#ef4444' : '#E19864' ?>'" onblur="this.style.borderColor='<?= isset($errors['rejection_reason_' . $event['id']]) ? '#ef4444' : 'rgba(255, 255, 255, 0.1)' ?>'">
                                        <button type="submit" style="background: transparent; border: 1.5px solid rgba(255, 255, 255, 0.1); color: #94a3b8; height: 42px; padding: 0 1.2rem; border-radius: 8px; font-weight: 600; font-size: 0.9rem; cursor: pointer; transition: all 0.2s ease;" onmouseover="this.style.borderColor='#ef4444'; this.style.color='#ef4444'; this.style.background='rgba(239, 68, 68, 0.1)'" onmouseout="this.style.borderColor='rgba(255, 255, 255, 0.1)'; this.style.color='#94a3b8'; this.style.background='transparent'">
                                            Reject
                                        </button>
                                    </div>
                    ...
                    <?php endforeach; ?>
                <?php else: ?>
                    <div style="text-align: center; padding: 4rem 2rem; background: rgba(30, 41, 59, 0.3); border: 2px dashed rgba(255, 255, 255, 0.05); border-radius: 14px; margin-top: 1rem;">
                        <div style="color: #cbd5e1; margin-bottom: 1.5rem; display: flex; justify-content: center;">
                        ...
                        <h3 style="color: #ffffff; margin: 0 0 0.5rem 0; font-size: 1.4rem; font-weight: 700;">You're all caught up!</h3>
                        <p style="color: #94a3b8; margin: 0; font-size: 1rem;">There are no pending evenement requests to review right now.</p>
                ...
                <?php if (!empty($rejectedRequests)): ?>
                    <div style="margin-top: 2rem; padding-top: 2rem; border-top: 1px solid rgba(255, 255, 255, 0.05);">
                        <h2 style="font-size: 1.4rem; font-weight: 700; color: #ffffff; margin: 0 0 1.25rem 0; display: flex; align-items: center; gap: 10px;">
                        ...
                        <div style="display: flex; flex-direction: column; gap: 1.25rem;">
                            <?php foreach ($rejectedRequests as $event): ?>
                                <div style="background: rgba(239, 68, 68, 0.05); border: 1.5px solid rgba(239, 68, 68, 0.2); border-radius: 14px; padding: 1.5rem; position: relative; transition: transform 0.2s ease;" onmouseover="this.style.transform='translateY(-2px)'" onmouseout="this.style.transform='translateY(0)'">
                                    
                                    <!-- Header: Teacher info -->
                                    <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 1.2rem;">
                                        <div style="display: flex; align-items: center; gap: 12px;">
                                            <div style="width: 44px; height: 44px; background: rgba(239, 68, 68, 0.1); color: #ef4444; border-radius: 10px; display: flex; align-items: center; justify-content: center; font-weight: 700; font-size: 1.2rem;">
                                                <?= strtoupper(substr($event['creator_name'] ?? 'T', 0, 1)) ?>
                                            </div>
                                            <div>
                                                <h4 style="margin: 0 0 2px 0; color: #ffffff; font-size: 1.05rem; font-weight: 700;"><?= htmlspecialchars($event['creator_name'] ?? 'Teacher') ?></h4>
                                    ...
                                    <h3 style="margin: 0 0 0.75rem 0; color: #ffffff; font-size: 1.1rem; font-weight: 600;">
                                        <?= htmlspecialchars(($event['titre'] ?? '') ?: ($event['title'] ?? '')) ?>
                                    </h3>
                                    
                                    <div style="background: rgba(30, 41, 59, 0.5); padding: 12px; border-radius: 8px; border-left: 4px solid #ef4444; margin-top: 10px;">
...
<div id="eventDetailsOverlay"
    ...
    <div id="eventDetailsModal"
         style="background:#0f172a; border-radius:22px; width:100%; max-width:580px;
                box-shadow:0 30px 80px rgba(0,0,0,0.4);
                transform:scale(0.92) translateY(20px);
                transition:transform 0.3s cubic-bezier(.34,1.56,.64,1), opacity 0.25s ease;
                opacity:0; overflow:hidden; border: 1px solid rgba(255, 255, 255, 0.1);">

        <!-- Modal Header -->
        <div style="background:rgba(30, 41, 59, 0.8); padding:1.8rem 2rem; display:flex; justify-content:space-between; align-items:flex-start; border-bottom: 1px solid rgba(255, 255, 255, 0.05);">
            <div>
        ...
        <div style="padding:1.8rem 2rem; display:flex; flex-direction:column; gap:1.1rem; max-height:65vh; overflow-y:auto;">
            ...
            <!-- Teacher Info -->
            <div style="background:rgba(255, 255, 255, 0.03); border-radius:12px; padding:1rem 1.2rem; display:flex; align-items:center; gap:12px; border:1px solid rgba(255, 255, 255, 0.05);">
                <div id="modal-avatar" style="width:42px; height:42px; border-radius:10px; background:linear-gradient(135deg,#E19864,#d9804b); color:#fff; display:flex; align-items:center; justify-content:center; font-weight:700; font-size:1.1rem; flex-shrink:0;"></div>
                <div>
                    <div id="modal-creator" style="font-weight:700; color:#ffffff; font-size:0.95rem;"></div>
                    <div id="modal-email" style="color:#94a3b8; font-size:0.82rem; margin-top:2px;"></div>
                </div>
                <div style="margin-left:auto;">
                    <span style="background:rgba(225, 152, 100, 0.15); color:#E19864; padding:4px 10px; border-radius:6px; font-size:0.75rem; font-weight:700; text-transform:uppercase;">Teacher</span>
                </div>
            </div>

            <!-- Date / Time / Location row -->
            <div style="display:grid; grid-template-columns:1fr 1fr 1fr; gap:10px;">
                <div style="background:rgba(255, 255, 255, 0.03); border-radius:10px; padding:0.9rem; border:1px solid rgba(255, 255, 255, 0.05); text-align:center;">
                    ...
                    <div id="modal-date-debut" style="color:#ffffff; font-weight:700; font-size:0.9rem;"></div>
                    <div id="modal-heure-debut" style="color:#E19864; font-size:0.82rem; font-weight:600; margin-top:2px;"></div>
                </div>
                <div style="background:rgba(255, 255, 255, 0.03); border-radius:10px; padding:0.9rem; border:1px solid rgba(255, 255, 255, 0.05); text-align:center;">
                    ...
                    <div id="modal-date-fin" style="color:#ffffff; font-weight:700; font-size:0.9rem;"></div>
                    <div id="modal-heure-fin" style="color:#E19864; font-size:0.82rem; font-weight:600; margin-top:2px;"></div>
                </div>
                <div style="background:rgba(255, 255, 255, 0.03); border-radius:10px; padding:0.9rem; border:1px solid rgba(255, 255, 255, 0.05); text-align:center;">
            ...
            <div style="display:grid; grid-template-columns:1fr 1fr; gap:10px;">
                <div style="background:rgba(255, 255, 255, 0.03); border-radius:10px; padding:0.9rem 1rem; border:1px solid rgba(255, 255, 255, 0.05); display:flex; align-items:center; gap:10px;">
                    ...
                    <div>
                        ...
                        <div id="modal-lieu" style="color:#ffffff; font-weight:600; font-size:0.88rem; margin-top:2px;"></div>
                    </div>
                </div>
                <div style="background:rgba(255, 255, 255, 0.03); border-radius:10px; padding:0.9rem 1rem; border:1px solid rgba(255, 255, 255, 0.05); display:flex; align-items:center; gap:10px;">
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#E19864" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M20.59 13.41l-7.17 7.17a2 2 0 0 1-2.83 0L2 12V2h10l8.59 8.59a2 2 0 0 1 0 2.82z"></path><line x1="7" y1="7" x2="7.01" y2="7"></line></svg>
                    <div>
                        <div style="color:#94a3b8; font-size:0.72rem; font-weight:700; text-transform:uppercase;">Type</div>
                        <div id="modal-type" style="color:#ffffff; font-weight:600; font-size:0.88rem; margin-top:2px;"></div>
                    </div>
                </div>
            </div>

            <!-- Description -->
            <div style="background:rgba(255, 255, 255, 0.03); border-radius:12px; padding:1.1rem 1.2rem; border:1px solid rgba(255, 255, 255, 0.05);">
                ...
                <p id="modal-description" style="margin:0; color:#cbd5e1; font-size:0.9rem; line-height:1.65;"></p>
            </div>
            ...
            <div style="display:flex; align-items:center; gap:6px; color:#94a3b8; font-size:0.82rem;">
                ...
                Submitted on: <span id="modal-created-at" style="font-weight:600; color:#cbd5e1;"></span>
            </div>
            ...
            <div style="background:rgba(255, 255, 255, 0.03); border-radius:12px; padding:1.1rem 1.2rem; border:1px solid rgba(255, 255, 255, 0.05);">
                ...
                <div style="display:flex; gap:6px; margin-bottom:10px; flex-wrap:wrap;">
                    <button id="res-tab-rule"
                        ...
                        style="display:flex; align-items:center; gap:5px; padding:5px 12px; border-radius:8px; border:1.5px solid rgba(225, 152, 100, 0.3); background:rgba(225, 152, 100, 0.15); color:#E19864; font-size:0.78rem; font-weight:700; cursor:pointer; transition:all 0.18s;">
                        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><line x1="8" y1="6" x2="21" y2="6"></line><line x1="8" y1="12" x2="21" y2="12"></line><line x1="8" y1="18" x2="21" y2="18"></line><line x1="3" y1="6" x2="3.01" y2="6"></line><line x1="3" y1="12" x2="3.01" y2="12"></line><line x1="3" y1="18" x2="3.01" y2="18"></line></svg>
                        Rules
                    </button>
                    <button id="res-tab-materiel"
                        onclick="switchResTab('materiel')"
                        style="display:flex; align-items:center; gap:5px; padding:5px 12px; border-radius:8px; border:1.5px solid rgba(255, 255, 255, 0.1); background:rgba(255, 255, 255, 0.05); color:#94a3b8; font-size:0.78rem; font-weight:700; cursor:pointer; transition:all 0.18s;">
        ...
        <!-- Modal Footer -->
        <div style="padding:1.2rem 2rem; border-top:1px solid rgba(255, 255, 255, 0.05); display:flex; justify-content:flex-end;">
            <button onclick="closeEventModal()" style="background:rgba(255, 255, 255, 0.05); border:1px solid rgba(255, 255, 255, 0.1); color:#cbd5e1; padding:0.6rem 1.5rem; border-radius:8px; font-weight:600; font-size:0.9rem; cursor:pointer; transition:background 0.2s;" onmouseover="this.style.background='rgba(255, 255, 255, 0.1)'" onmouseout="this.style.background='rgba(255, 255, 255, 0.05)'">Close</button>
        </div>
    </div>
</div>
...
function renderResPanel(type, items) {
    ...
    if (!items || items.length === 0) {
        panel.innerHTML = '<div style="text-align:center; padding:1rem; color:#94a3b8; font-size:0.83rem; background:rgba(30, 41, 59, 0.3); border-radius:8px; border:1.5px dashed rgba(255, 255, 255, 0.05);">No ' + (type === 'materiel' ? 'materials' : type + 's') + ' added yet.</div>';
        return;
    }
    var html = '<div style="display:flex; flex-direction:column; gap:6px;">';
    var accentColors = {rule:'#548CA8', materiel:'#E19864', plan:'#355C7D'};
    items.forEach(function(item) {
        var accent = accentColors[type] || '#E19864';
        html += '<div style="background:rgba(255, 255, 255, 0.03); border:1px solid rgba(255, 255, 255, 0.05); border-left:3px solid ' + accent + '; border-radius:8px; padding:0.7rem 0.9rem;">';
        html +=   '<div style="font-weight:700; color:#ffffff; font-size:0.87rem;">' + escHtml(item.title) + '</div>';
        if (item.details) {
            html += '<div style="color:#94a3b8; font-size:0.81rem; margin-top:3px; line-height:1.55;">' + escHtml(item.details) + '</div>';
        }
        html += '</div>';
    });
    html += '</div>';
...
function switchResTab(active) {
    ['rule','materiel','plan'].forEach(function(t) {
        ...
        if (!tab || !panel) return;
        if (t === active) {
            tab.style.background  = 'rgba(225, 152, 100, 0.15)';
            tab.style.color       = '#E19864';
            tab.style.borderColor = 'rgba(225, 152, 100, 0.3)';
