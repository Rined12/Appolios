        ...
        <div class="admin-layout">
            ...
            <div class="admin-main">
                <div class="dashboard-header" style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:12px;">
                    ...
                    <a href="<?= APP_ENTRY ?>?url=admin/add-question" class="btn btn-yellow">Nouvelle question</a>
                </div>
                <?php if (!empty($flash)): ?>
                    ...
                    <div style="padding:16px;">
                        <?php if (!empty($questions)): foreach ($questions as $q): ?>
                            <div style="padding:16px;border-bottom:1px solid var(--gray);">
                                ...
                                <a href="<?= APP_ENTRY ?>?url=admin/edit-question/<?= (int) $q['id'] ?>" class="btn btn-secondary" style="padding:6px 12px;">Modifier</a>
                                <a href="<?= APP_ENTRY ?>?url=admin/delete-question/<?= (int) $q['id'] ?>" class="btn action-btn danger" style="padding:6px 12px;" onclick="return confirm('Supprimer ?');">Supprimer</a>
                            </div>
                        <?php endforeach; else: ?>
                            <p>Aucune question.</p>
                        <?php endif; ?>
