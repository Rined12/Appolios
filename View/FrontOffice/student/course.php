                    ...
                    <div style="display: flex; gap: 1rem; margin-bottom: 1.5rem; flex-wrap: wrap;">
                        <div style="flex: 1; min-width: 280px; background: white; border-radius: 12px; padding: 1rem;">
                            ...
                            <div style="display: flex; justify-content: space-between; align-items: center; margin-top: 0.5rem;">
                                <p id="lessons-completed" style="margin: 0; font-size: 0.85rem; color: #64748b;"><?= count($completedLessons ?? []) ?> of <?= $course['lesson_count'] ?? 0 ?> lessons</p>
                                <?php if (!empty($certificate)): ?>
                                    <a href="<?= APP_ENTRY ?>?url=student/downloadCertificate/<?= htmlspecialchars($certificate['certificate_code']) ?>" style="background: linear-gradient(135deg, #f59e0b 0%, #d97706 100%); color: white; padding: 4px 10px; border-radius: 6px; font-size: 0.75rem; font-weight: 600; text-decoration: none; display: flex; align-items: center; gap: 4px;">
                                        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 15l-8-4.5V21l8 4.5 8-4.5v-10.5L12 15z"/><path d="M12 3L4 7.5 12 12l8-4.5L12 3z"/></svg>
                                        Certificate
                                    </a>
                                <?php endif; ?>
                            </div>
                                            ...
                                            <div onclick="toggleAndComplete(<?= $chapterIndex ?>, <?= $lessonIndex ?>, <?= $lesson['id'] ?>, <?= $course['id'] ?>)" style="padding: 1rem; cursor: pointer; display: flex; justify-content: space-between; align-items: center;">
                                                <div style="display: flex; align-items: center; gap: 0.75rem;">
                                                    ...
                                                    <div>
                                                        ...
                                                        <p style="margin: 0.25rem 0 0; font-size: 0.85rem; color: #64748b;"><?= ucfirst($lesson['lesson_type'] ?? 'text') ?></p>
                                                    </div>
                                                </div>
                                                <?php if ($isCompleted): ?>
...
function markComplete(lid, cid, chapterIndex, lessonIndex) {
    ...
    x.onreadystatechange = function() {
        if (x.readyState === 4 && x.status === 200) {
            ...
            } catch(e) { console.log(x.responseText); }
...
function updateLessonUI(chapterIndex, lessonIndex) {
    ...
    var innerDiv = lessonDiv.querySelector('div:first-child');
    if (!innerDiv) return;
    
    var leftSide = innerDiv.querySelector('div:first-child');
    if (leftSide) {
        leftSide.innerHTML = '<svg width="18" height="18" viewBox="0 0 24 24" fill="#22c55e" stroke="#22c55e" stroke-width="2"><path d="M22 11.08V12a10 10 0 11-5.93-9.44"/><polyline points="22 4 12 14.01 9 11.01"/></svg><div><h4 style="margin: 0; font-size: 1rem; color: #16a34a;">' + lessonDiv.querySelector('h4').textContent + '</h4><p style="margin: 0.25rem 0 0; font-size: 0.85rem; color: #64748b;">' + lessonDiv.querySelector('p').textContent + '</p></div>';
    }
    
    var rightSide = innerDiv.querySelector('svg');
    if (rightSide && rightSide.id && rightSide.id.startsWith('lesson-icon')) {
