...
$old = $old ?? [];
$errors = $errors ?? [];
$coverRaw = trim((string) ($groupe['image_url'] ?? $groupe['photo'] ?? $groupe['image'] ?? ''));
$cover = '';
if ($coverRaw !== '') {
...
<?php require __DIR__ . '/../partials/collab_layout_start.php'; ?>
                ...
                <div class="header collab-hero" style="margin-bottom:1.2rem;">
                    <div class="collab-hero__inner">
                        ...
                        <div class="collab-hero-actions">
                            <a href="<?= APP_ENTRY ?>?url=<?= htmlspecialchars($foPrefix, ENT_QUOTES, 'UTF-8') ?>/groupes/<?= (int) $groupe['id_groupe'] ?>" class="collab-btn-ghost">
                ...
                <div style="display:grid;grid-template-columns:1.1fr 0.9fr;gap:1.1rem;align-items:start;" class="group-edit-grid">
                    <div class="section collab-form-shell" style="max-width:none;">
                        <form method="POST" action="<?= APP_ENTRY ?>?url=<?= htmlspecialchars($foPrefix, ENT_QUOTES, 'UTF-8') ?>/groupes/<?= (int) $groupe['id_groupe'] ?>/update" enctype="multipart/form-data">
