                ...
                <div style="background: url('https://images.unsplash.com/photo-1540575467063-178a50c2df87?auto=format&fit=crop&q=80&w=1200') center/cover no-repeat; border-radius: 20px; margin-bottom: 2rem; color: white; position: relative; overflow: hidden; box-shadow: 0 10px 30px rgba(43, 72, 101, 0.15); min-height: 250px; display: flex; align-items: flex-end;">
                    ...
                    <div style="position: relative; z-index: 1; padding: 2.5rem; width: 100%; display: flex; justify-content: space-between; align-items: flex-end; flex-wrap: wrap; gap: 1.5rem;">
                        ...
                        <div style="display: flex; gap: 12px; flex-wrap: wrap;">
                            ...
                            <a href="<?= APP_ENTRY ?>?url=teacher/stat-evenements" style="background: #548CA8; color: white; text-decoration: none; padding: 12px 24px; border-radius: 10px; font-weight: 600; box-shadow: 0 4px 15px rgba(84, 140, 168, 0.3); transition: all 0.2s; display: inline-flex; align-items: center; gap: 8px;" onmouseover="this.style.transform='translateY(-2px)'; this.style.boxShadow='0 6px 20px rgba(84, 140, 168, 0.4)'" onmouseout="this.style.transform='translateY(0)'; this.style.boxShadow='0 4px 15px rgba(84, 140, 168, 0.3)'">
                        ...
                        <div style="display: flex; gap: 1rem; align-items: center;">
                            <div class="custom-select-wrapper" style="position: relative; user-select: none; width: 170px; z-index: 50;">
                                ...
                                <div class="custom-select-options" style="position: absolute; top: calc(100% + 8px); left: 0; width: 100%; background: white; border: 1px solid #eef2f6; border-radius: 12px; box-shadow: 0 10px 30px rgba(0,0,0,0.1); opacity: 0; visibility: hidden; transform: translateY(-10px); transition: all 0.2s; overflow: hidden; padding: 6px;">
                                    ...
                                    <div class="custom-option" data-value="dateDesc" style="padding: 10px 15px; cursor: pointer; border-radius: 8px; font-size: 0.9rem; color: #475569; transition: all 0.2s; margin-bottom: 2px;" onmouseover="this.style.background='#f1f5f9'; this.style.color='#1e293b'" onmouseout="this.style.background='transparent'; this.style.color='#475569'">Date (Newest)</div>
                                </div>
                            </div>
                                ...
                                tr.innerHTML = `
                                    ...
                                    <td style="padding: 1rem; text-align: right;">
                                        <div style="display: flex; gap: 5px; justify-content: flex-end; align-items: center;">
                                            ...
                                            ${currentStatus === 'pending' && (p.event_creator_id == <?= (int)$_SESSION['user_id'] ?>) ? `
                                            ...
                                            ` : (currentStatus === 'pending') ? `
                                                <span style="background:#f1f5f9;color:#94a3b8;padding:4px 8px;border-radius:6px;font-size:0.72rem;font-weight:600;display:inline-block;">View Only</span>
                                            ` : ``}
                                        </div>
                                    </td>
                                `;
...
function aiPredictEvents(role) {
    ...
    fetch('<?= APP_ENTRY ?>?url=event/predict-top-events&role=' + role)
        ...
        .then(data => {
            ...
            // Even if there is an ai_error (like quota), if we have events (fallback), show them!
            if(data.success && data.events && data.events.length > 0) {
                ...
                resultsContainer.innerHTML = '<div style="text-align:center; padding: 2rem; color: #dc2626; background: #fef2f2; border-radius: 12px;">' + (data.message || 'No predictions available at this moment.') + '</div>';
            }
        })
        .catch(err => {
            ...
            loader.style.display = 'none';
            resultsContainer.style.display = 'flex';
            resultsContainer.innerHTML = '<div style="text-align:center; padding: 2rem; color: #dc2626; background: #fef2f2; border-radius: 12px;">Connection error. Please try again later.</div>';
        });
}
