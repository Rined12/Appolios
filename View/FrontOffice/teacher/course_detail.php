                                ...
                                <div id="chapter-content-<?= $chapterIndex ?>" style="display: none; margin-top: 15px; padding-left: 10px;">
                                    ...
                                    <?php if (!empty($chapter['lessons'])): ?>
                                        ...
                                        <?php foreach ($chapter['lessons'] as $lessonIndex => $lesson): ?>
                                            <?php $lessonType = $lesson['lesson_type'] ?? 'text'; ?>
                                            <?php $lessonColor = $lessonType === 'text' ? '#10b981' : ($lessonType === 'pdf' ? '#ef4444' : '#f59e0b'); ?>
...
html += '<span style="font-size: 0.8rem; color: #64748b;">Type: ' + (lesson.lesson_type || 'text') + '</span>';
    
    if ((lesson.lesson_type || 'text') === 'text' && lesson.content) {
        html += '<div style="background: white; border: 1px solid #e2e8f0; border-radius: 8px; padding: 20px; margin-bottom: 15px;">';
        html += lesson.content;
        html += '</div>';
    } else if ((lesson.lesson_type || 'text') === 'pdf' && lesson.pdf_file) {
