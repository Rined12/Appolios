            ...
            <div class="admin-main">
                <div class="dashboard-header" style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:12px;">
                    ...
                    <div style="display:flex;gap:10px;flex-wrap:wrap;">
                        <a href="<?= APP_ENTRY ?>?url=teacher/chapitres-add" class="btn btn-yellow">Nouveau chapitre</a>
                        <a href="<?= APP_ENTRY ?>?url=teacher/courses" class="btn btn-outline">Retour aux cours</a>
                    </div>
                </div>
                <?php if (!empty($flash)): ?>
                        ...
                        <table class="data-table" style="width:100%;">
                            ...
                            <?php if (!empty($chapters)): foreach ($chapters as $ch): ?>
                                <tr>
                                    ...
                                    <td>
                                        <a href="<?= APP_ENTRY ?>?url=teacher/edit-chapter/<?= (int) $ch['id'] ?>" class="btn btn-secondary" style="padding:6px 10px;font-size:0.8rem;">Modifier</a>
                                        <a href="<?= APP_ENTRY ?>?url=teacher/delete-chapter/<?= (int) $ch['id'] ?>" class="btn action-btn danger" style="padding:6px 10px;font-size:0.8rem;" onclick="return confirm('Supprimer ce chapitre ?');">Supprimer</a>
                                    </td>
                                </tr>
                            <?php endforeach; else: ?>
                                <tr><td colspan="5">Aucun chapitre. Utilisez « Nouveau chapitre » pour en ajouter.</td></tr>
                            <?php endif; ?>
                            </tbody>
                        </table>
                    </div>
