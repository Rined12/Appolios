...
$courseProgress = isset($courseProgress) ? max(0, min(100, (int) $courseProgress)) : 0;
$courseQuizCount = isset($courseQuizCount) ? (int) $courseQuizCount : 0;
$attemptedCourseQuizCount = isset($attemptedCourseQuizCount) ? (int) $attemptedCourseQuizCount : 0;
?>
            ...
            <aside class="neo-main-stack">
                <section class="neo-glass-card neo-profile-card">
                    ...
                    <div class="neo-progress-wrap" style="margin-top: 0.8rem;">
                        <div class="neo-progress-track"><div class="neo-progress-value" style="width: <?= $isEnrolled ? $courseProgress : 0 ?>%;"></div></div>
                        <?php if ($isEnrolled): ?>
                            <div class="neo-progress-meta">
                                <?= $courseProgress ?>% completed
                                <?php if ($courseQuizCount > 0): ?>
                                    (<?= $attemptedCourseQuizCount ?>/<?= $courseQuizCount ?> quiz started)
                                <?php else: ?>
                                    (No quiz available yet)
                                <?php endif; ?>
                            </div>
                        <?php else: ?>
                            <div class="neo-progress-meta">Enroll to start progress</div>
                        <?php endif; ?>
                    </div>
                    <?php if ($isEnrolled): ?>
                        <a class="neo-btn neo-btn-primary" style="margin-top: 0.9rem; width: 100%;" href="<?= APP_ENTRY ?>?url=student/chapitres">Continue Lesson</a>
                    <?php else: ?>
