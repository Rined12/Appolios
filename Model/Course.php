...
class Course extends BaseModel {
    ...
    public static function getAllWithCreator(): array {
        ...
        $stmt = $db->query("
            ...
            (SELECT COUNT(*) FROM lessons WHERE chapter_id IN (SELECT id FROM chapters WHERE course_id = c.id)) as lessons_count
            FROM courses c
    ...
    public static function countByStatus(string $status): int {
        $db = getConnection();
        $stmt = $db->prepare("SELECT COUNT(*) as count FROM courses WHERE status = ?");
        $stmt->execute([$status]);
        $result = $stmt->fetch();
        return (int) ($result['count'] ?? 0);
    }

    public static function updateStatus(int $id, string $status, ?string $adminMessage = null): bool {
        $db = getConnection();
        $stmt = $db->prepare("UPDATE courses SET status = ?, admin_message = ? WHERE id = ?");
