...
class RessourceController extends BaseController {
    ...
    public function approveParticipation($id) {
        ...
        $participation = $this->queryFindRessource((int) $id);

        if (!$participation || $participation['type'] !== 'participation') {
    ...
    public function generateAiResources() {
        ...
        foreach ($apiResponse['rules'] as $r) {
            $this->queryCreate([
        ...
        foreach ($apiResponse['materiels'] as $m) {
