...
class CourseController extends BaseModel {
    ...
    public function getCoursesByTeacher($teacherId) {
        ...
        try {
            ...
            if (str_contains($e->getMessage(), "Table '" . DB_NAME . ".lessons' doesn't exist")) {
                $fallbackSql = "SELECT c.*,
                        (SELECT COUNT(*) FROM chapters ch WHERE ch.course_id = c.id) as chapter_count,
                        0 as lesson_count
                        FROM {$this->table} c
                        WHERE c.created_by = ?
    ...
    public function getTeacherMonthlyEarnings($teacherId, $range = 'month') {
        ...
        try {
            $countStmt = $this->db->query('SELECT COUNT(*) AS cnt FROM payments');
            $countRow = $countStmt ? $countStmt->fetch() : null;
            $paymentsCount = (int) ($countRow['cnt'] ?? 0);

            if ($paymentsCount === 0) {
                $courseStmt = $this->db->prepare('SELECT id FROM courses WHERE created_by = ? ORDER BY id ASC LIMIT 1');
                $courseStmt->execute([(int) $teacherId]);
                $courseRow = $courseStmt->fetch();
                $courseId = (int) ($courseRow['id'] ?? 0);

                $userStmt = $this->db->query("SELECT id FROM users WHERE role = 'student' ORDER BY id ASC LIMIT 1");
                ...
                if ($courseId > 0 && $studentId > 0) {
                    $samplePayments = [
                        ...
                        ['amount' => 75.00, 'date' => '2026-04-15'],
                        ['amount' => 100.00, 'date' => '2026-04-20'],
                        ['amount' => 25.00, 'date' => '2026-03-05'],
                        ['amount' => 150.00, 'date' => '2026-03-18'],
                        ['amount' => 80.00, 'date' => '2026-02-10'],
                        ['amount' => 60.00, 'date' => '2026-02-25'],
                        ['amount' => 90.00, 'date' => '2026-01-08'],
                        ['amount' => 45.00, 'date' => '2026-01-22'],
                        ['amount' => 120.00, 'date' => '2025-12-12'],
                        ['amount' => 55.00, 'date' => '2025-12-28'],
                        ['amount' => 200.00, 'date' => '2025-11-05'],
                    ];
                    ...
                    foreach ($samplePayments as $p) {
                        $insStmt->execute([$studentId, $courseId, 'demo_' . uniqid(), $p['amount'], $p['date'] . ' 10:00:00']);
                    }
                }
            }
        } catch (PDOException $e) {
            // If demo inserts fail for any reason, ignore and continue with real payments.
        }
    ...
    public function getWithChapters($id) {
        ...
        $chapters = $chapterController->getByCourseId($id);
        
        foreach ($chapters as &$chapter) {
            $chapter['lessons'] = $lessonController->getByChapterId($chapter['id']);
        }
        
        $course['chapters'] = $chapters;
        $course['lesson_count'] = $lessonController->getLessonCount($id);
        $course['total_duration'] = $lessonController->getTotalDuration($id);
