...
class AuthController extends BaseController {
    ...
    public function signup() {
        ...
        if ($role === 'teacher') {
            ...
            // Create user first (as student pending approval)
            ...
            if (!$userId) {
                unlink($cvPath);
                $this->setFlash('error', 'Failed to create user account. Please try again.');
                $_SESSION['old'] = ['name' => $name, 'email' => $email];
                $this->redirect('register');
                return;
            }
            ...
            $appId = $teacherAppModel->createApplication([
                'user_id' => $userId,
                'motivation' => 'Teacher application from registration',
            ...
            if ($appId) {
                ...
                unlink($cvPath);
                $this->setFlash('error', 'Account created but failed to submit application. Please contact support.');
                $this->redirect('login');
            }
            return;
        }
