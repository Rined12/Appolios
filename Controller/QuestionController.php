...
class QuestionController extends BaseController
{
    ...
    private function teacherStoreQuestion(): void
    {
        ...
        $newId = $this->queryCreateQuestionBankQuestion((int) $_SESSION['user_id'], $title !== '' ? $title : null, $questionText, $opts, $correct, $tags !== '' ? $tags : null, $difficulty);
        $this->setFlash('success', 'Question enregistrée.');
        if ($newId) {
            $this->redirect('teacher-quiz/edit-question/' . $newId);
        } else {
            $this->redirect('teacher-quiz/add-question');
        }
    }
    ...
    private function teacherUpdateQuestion($id): void
    {
        ...
        $this->redirect('teacher-quiz/edit-question/' . (int) $id);
    }
    ...
    {
        $db = $this->getDb();
        $sql = "INSERT INTO question_bank (created_by, title, question_text, options_json, correct_answer, tags, difficulty)
                VALUES (?, ?, ?, ?, ?, ?, ?)";
        $stmt = $db->prepare($sql);
        $json = json_encode(is_array($options) ? array_values($options) : []);
        $ok = $stmt->execute([
