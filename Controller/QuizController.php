...
- TRÈS IMPORTANT: Sois extrêmement concis dans le texte des questions et des options. Ne mets aucune explication ni justification. La longueur totale du JSON ne doit pas dépasser les limites de l'API.
...
";
    ...
    public function generateSingleQuestionAi()
    {
        $this->requireAdminOrTeacher();
        ...
        if (trim((string) GEMINI_API_KEY) === '') {
            echo json_encode(['ok' => false, 'error' => 'Clé Gemini manquante.']);
            exit;
        }
        ...
        if ($topic === '') {
            echo json_encode(['ok' => false, 'error' => 'Le sujet est requis.']);
            exit;
        }

        try {
            $prompt = "Tu es un expert pédagogique. Génère EXACTEMENT UNE question au format JSON strict sur le sujet : '{$topic}'.
Niveau de difficulté ciblé : {$difficulty}.

La question doit être un QCM (choix multiple).
- Fournis exactement 4 options.
- L'index de la réponse correcte doit être entre 0 et 3.
- Fournis quelques tags pertinents.
- SOIS TRÈS CONCIS dans les textes pour éviter que la réponse ne soit coupée. Ne mets pas de formatage markdown (pas de ```json), retourne uniquement l'objet JSON commençant par { et finissant par }.
...
Format JSON attendu :
{
  ...
  \"tags\": \"tag1, tag2\"
            ...
            $rawText = $this->callGeminiGenerateText($prompt);
            $parsed = $this->parseGeminiJson($rawText);

            if (isset($parsed['question_text']) && isset($parsed['options'])) {
                $q = $parsed;
            } elseif (isset($parsed['questions']) && is_array($parsed['questions']) && !empty($parsed['questions'])) {
                $q = $parsed['questions'][0];
            } else {
                echo json_encode(['ok' => false, 'error' => 'Format de réponse IA non reconnu.']);
                exit;
            }
    ...
    private function parseGeminiJson(string $rawText): array
    {
        ...
        throw new RuntimeException('Réponse IA invalide (JSON). Longueur totale: ' . strlen($t) . ' Extrait: ' . $snippet);
    }
    ...
    public function remediationPlan()
    {
        ...
        $this->view('FrontOffice/teacher/remediation_plan', [
            'title' => 'Plan de rattrapage - ' . APP_NAME,
            'items' => $items,
            'flash' => $this->getFlash(),
        ]);
    }
    ...
    {
        if (!$this->isLoggedIn() || !in_array(($_SESSION['role'] ?? ''), ['admin', 'teacher'], true)) {
            $this->setFlash('error', 'Accès refusé.');
            $this->redirect('login');
        }
    }
...
";
    ...
    private function callGeminiGenerateText(string $prompt): string
    {
        ...
        foreach ($models as $model) {
            ...
            for ($attempt = 1; $attempt <= 3; $attempt++) {
                ...
                if ($resp === false || $code < 200 || $code >= 300) {
                    $detail = $err !== '' ? $err : ($useOpenRouter ? 'OpenRouter HTTP error' : 'Gemini HTTP error');
                    if ($resp !== false) {
                    ...
                    if (in_array($code, [429, 503], true)) {
                        ...
                        // After 3 attempts on this model, break to try the next model in the list
                        break;
                    }
                    break;
                }
                ...
                if (!is_array($decoded)) {
                    $lastError = '[' . $model . '] ' . ($useOpenRouter ? 'OpenRouter' : 'Gemini') . ' response not JSON';
                    break;
                }
                ...
                if (trim((string) $text) === '') {
                    $lastError = '[' . $model . '] ' . ($useOpenRouter ? 'OpenRouter' : 'Gemini') . ' empty response';
                    break;
                }
        ...
        throw new RuntimeException($lastError !== '' ? $lastError : 'Toutes les tentatives IA ont échoué.');
    }
    ...
    {
        $db = $this->getDb();
        $sql = 'SELECT ch.*, c.title AS course_title FROM chapters ch JOIN courses c ON c.id = ch.course_id ORDER BY c.title ASC, ch.id ASC, ch.title ASC';
        $stmt = $db->query($sql);
    ...
    private function queryChaptersForTeacher(int $teacherId): array
    {
        ...
        $chapterOrderColumn = 'id';
        if ($this->tableHasColumn('chapters', 'chapter_order')) {
            $chapterOrderColumn = 'chapter_order';
        } elseif ($this->tableHasColumn('chapters', 'sort_order')) {
            $chapterOrderColumn = 'sort_order';
        }
        ...
        $sql = 'SELECT ch.*, c.title AS course_title
                ...
                JOIN courses c ON c.id = ch.course_id
                WHERE c.created_by = ?
                ORDER BY c.title ASC, ch.' . $chapterOrderColumn . ' ASC';
        $st = $db->prepare($sql);
        $st->execute([(int) $teacherId]);
        return $st->fetchAll();
    }
    ...
    private function queryQuizzesForEnrolledStudent(int $studentId): array
    {
        ...
        $sql = "SELECT q.*, ch.title AS chapter_title, ch.id AS chapter_id, c.title AS course_title, c.id AS course_id
                ...
                ORDER BY c.title ASC, ch.id ASC, q.created_at DESC";
        $stmt = $db->prepare($sql);
