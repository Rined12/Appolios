<?php
/**
 * APPOLIOS Lesson Progress Controller
 * Handles lesson progress-related business logic
 */

require_once __DIR__ . '/../Model/BaseModel.php';

class LessonProgressController extends BaseModel {
    ...
    public function markComplete($userId, $lessonId) {
        ...
        if ($exists) {
            $sql = "UPDATE {$this->table} SET is_completed = 1, completed_at = NOW() WHERE user_id = ? AND lesson_id = ?";
            $stmt = $this->db->prepare($sql);
            return $stmt->execute([$userId, $lessonId]);
        } else {
            $sql = "INSERT INTO {$this->table} (user_id, lesson_id, is_completed, completed_at) VALUES (?, ?, 1, NOW())";
            $stmt = $this->db->prepare($sql);
            return $stmt->execute([$userId, $lessonId]);
        }
    }
    ...
    public function markCompleteNoDuplicate($userId, $lessonId) {
        $sql = "INSERT INTO {$this->table} (user_id, lesson_id, is_completed, completed_at) 
                VALUES (?, ?, 1, NOW())
    ...
    public function markIncomplete($userId, $lessonId) {
        $sql = "UPDATE {$this->table} SET is_completed = 0, completed_at = NULL WHERE user_id = ? AND lesson_id = ?";
        $stmt = $this->db->prepare($sql);
        return $stmt->execute([$userId, $lessonId]);
    }
    ...
    public function isLessonCompleted($userId, $lessonId) {
        ...
        return $progress && $progress['is_completed'] == 1;
    }
