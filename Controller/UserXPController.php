...
class UserXPController extends BaseModel {
    ...
    private function createTables() {
        ...
        try {
            $this->db->exec("ALTER TABLE user_xp ADD COLUMN total_xp INT DEFAULT 0");
    ...
    public function addXP($userId, $amount, $reason = '') {
        ...
        if ($current) {
            $sql = "UPDATE {$this->table} SET xp = xp + ?, total_xp = total_xp + ?, updated_at = NOW() WHERE user_id = ?";
            $stmt = $this->db->prepare($sql);
            $stmt->execute([$amount, $amount, $userId]);
        } else {
            $sql = "INSERT INTO {$this->table} (user_id, xp, total_xp, updated_at) VALUES (?, ?, ?, NOW())";
            $stmt = $this->db->prepare($sql);
