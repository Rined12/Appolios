...
class UserXPController extends BaseModel {
    ...
    public function addXP($userId, $amount, $reason = '') {
        ...
        if ($current) {
            $sql = "UPDATE {$this->table} SET xp = xp + ?, xp_points = xp_points + ?, total_xp = total_xp + ?, last_xp_at = NOW() WHERE user_id = ?";
            $stmt = $this->db->prepare($sql);
            $stmt->execute([$amount, $amount, $amount, $userId]);
        } else {
            $sql = "INSERT INTO {$this->table} (user_id, xp, xp_points, total_xp, last_xp_at) VALUES (?, ?, ?, ?, NOW())";
            $stmt = $this->db->prepare($sql);
            $stmt->execute([$userId, $amount, $amount, $amount]);
        }
