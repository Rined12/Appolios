...
class StudentController extends BaseController {
    ...
    public function completeLesson($lessonId = null, $courseId = null) {
        ...
        try {
            $this->checkGlobalBadges($_SESSION['user_id']);
        } catch (Exception $e) {
            error_log('Global badge check failed: ' . $e->getMessage());
        }
    ...
    private function checkAndAwardBadges($userId, $courseId, $completedCount, $totalLessons, $progress) {
        ...
        if ($progress >= 100) {
            error_log("BADGE CHECK: progress=$progress, courseId=$courseId");
            $badge = null;
            try {
                ...
                if ($badge) {
                    ...
                    if (!$badgeCtrl->hasBadge($userId, $badge['name'])) {
                        $badgeCtrl->awardBadge($userId, $badge['name'], $badge['icon'], $badge['description']);
                ...
                error_log('AI Badge generation failed: ' . $e->getMessage());
            }
    ...
    /**
     * Check and award global badges based on total progress
     */
    private function checkGlobalBadges($userId) {
        require_once __DIR__ . '/../Controller/BadgeController.php';
        ...
        $progressCtrl = new LessonProgressController();
        $enrollmentCtrl = new EnrollmentController();
        $xpCtrl = new UserXPController();
        
        $userXP = $xpCtrl->getByUserId($userId);
        $xpPoints = $userXP['xp_points'] ?? $userXP['total_xp'] ?? 0;
        
        $userEnrollments = $enrollmentCtrl->getUserEnrollments($userId);
        $totalCourses = 0;
        $totalLessons = 0;
        ...
        foreach ($userEnrollments as $enrollment) {
            ...
            $completed = $progressCtrl->getCompletedCount($userId, $enrollment['course_id']);
            $totalLessons += is_array($completed) ? count($completed) : $completed;
        }
        
        $masterBadges = [
            ['name' => 'First Steps', 'icon' => '👣', 'criteria' => 'lessons_completed', 'value' => 1],
            ['name' => 'Quick Learner', 'icon' => '⚡', 'criteria' => 'lessons_completed', 'value' => 5],
            ['name' => 'Course Master', 'icon' => '🎓', 'criteria' => 'courses_completed', 'value' => 1],
            ['name' => 'Dedicated Student', 'icon' => '📚', 'criteria' => 'courses_completed', 'value' => 3],
            ['name' => 'XP Champion', 'icon' => '⭐', 'criteria' => 'xp_earned', 'value' => 500]
        ];
        
        foreach ($masterBadges as $badge) {
            $earned = false;
            
            switch ($badge['criteria']) {
                case 'lessons_completed':
                    $earned = ($totalLessons >= $badge['value']);
                    break;
                case 'courses_completed':
                    $earned = ($totalCourses >= $badge['value']);
                    break;
                case 'xp_earned':
                    $earned = ($xpPoints >= $badge['value']);
                    break;
            }
            
            if ($earned && !$badgeController->hasBadge($userId, $badge['name'])) {
                $badgeController->awardBadge($userId, $badge['name'], $badge['icon'], 'Earned: ' . $badge['name']);
                $this->createNotification(
                    $userId,
                    'badge',
                    '🎉 Global Badge Earned!',
