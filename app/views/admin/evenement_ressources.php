<?php
/**
 * APPOLIOS - Evenement Resources Workspace
 */

$old = $_SESSION['old'] ?? [];
unset($_SESSION['old']);
$selectedEvenementId = (int) ($selectedEvenementId ?? 0);
$selectedEvenementTitle = $selectedEvenement['title'] ?? '';
    ...
    <div class="container">
                <div class="dashboard-header" style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 15px;">
                    <div>
                        ...
                        <p>Add rules, materiel, and day plans in one workspace</p>
                ...
                <div class="ressource-workspace">
                    <div class="ressource-forms-col">
                        <div class="ressource-form-card">
                            <h3>Ajouter Rule</h3>
                            <form action="<?= APP_URL ?>/index.php?url=admin/store-evenement-ressource" method="POST" class="resource-create-form" data-type-label="Rule">
                        ...
                        <div class="ressource-form-card">
                            ...
                            <form action="<?= APP_URL ?>/index.php?url=admin/store-evenement-ressource" method="POST" class="resource-create-form" data-type-label="Materiel">
                        ...
                        <div class="ressource-form-card">
                            ...
                            <form action="<?= APP_URL ?>/index.php?url=admin/store-evenement-ressource" method="POST" class="resource-create-form" data-type-label="Plan">
                        ...
                        <div class="ressource-list-card">
                            ...
                            <ul class="ressource-list">
                                <?php if (!empty($rules)): ?>
                                    <?php foreach ($rules as $item): ?>
                                        <li>
                                            <?php if ($editResource && (int) $editResource['id'] === (int) $item['id']): ?>
                                                <form action="<?= APP_URL ?>/index.php?url=admin/update-evenement-ressource/<?= (int) $item['id'] ?>" method="POST">
                                                    <input type="hidden" name="evenement_id" value="<?= (int) $selectedEvenementId ?>">
                                                    <div class="form-group" style="margin-bottom: 8px;">
                                                        <input type="text" name="title" value="<?= htmlspecialchars($item['title']) ?>" required>
                                                    </div>
                                                    <div class="form-group" style="margin-bottom: 8px;">
                                                        <textarea name="details" placeholder="Rule details..."><?= htmlspecialchars($item['details'] ?? '') ?></textarea>
                                                    </div>
                                                    <button type="submit" class="btn btn-primary action-btn">Sauvegarder</button>
                                                    <a href="<?= APP_URL ?>/index.php?url=admin/evenement-ressources&evenement_id=<?= (int) $selectedEvenementId ?>" class="btn btn-outline action-btn">Annuler</a>
                                                </form>
                                            <?php else: ?>
                                                <strong><?= htmlspecialchars($item['title']) ?></strong>
                                                <p class="ressource-event-label">Event: <?= htmlspecialchars($item['evenement_title'] ?? 'Not linked') ?></p>
                                                <?php if (!empty($item['details'])): ?><p><?= htmlspecialchars($item['details']) ?></p><?php endif; ?>
                                                <div class="ressource-actions">
                                                    <a href="<?= APP_URL ?>/index.php?url=admin/evenement-ressources&evenement_id=<?= (int) $selectedEvenementId ?>&edit_id=<?= (int) $item['id'] ?>" class="btn btn-secondary action-btn">Edit</a>
                                                    <form action="<?= APP_URL ?>/index.php?url=admin/delete-evenement-ressource/<?= (int) $item['id'] ?>" method="POST" style="display: inline-block;">
                                                        <input type="hidden" name="evenement_id" value="<?= (int) $selectedEvenementId ?>">
                                                        <button type="submit" class="btn action-btn danger" data-confirm="Delete this rule?">Delete</button>
                                                    </form>
                                                </div>
                                            <?php endif; ?>
                                        </li>
                                    <?php endforeach; ?>
                                <?php else: ?>
                                    <li class="empty">No rule added yet.</li>
                                <?php endif; ?>
                            </ul>
                        </div>

                        <div class="ressource-list-card">
                            <h3>Liste Materiels</h3>
                            <ul class="ressource-list">
                                <?php if (!empty($materials)): ?>
                                        ...
                                        <li>
                                            <?php if ($editResource && (int) $editResource['id'] === (int) $item['id']): ?>
                                                <form action="<?= APP_URL ?>/index.php?url=admin/update-evenement-ressource/<?= (int) $item['id'] ?>" method="POST">
                                                    ...
                                                    <div class="form-group" style="margin-bottom: 8px;">
                                                        <textarea name="details" placeholder="Materiel details..."><?= htmlspecialchars($item['details'] ?? '') ?></textarea>
                                                    </div>
                                                    <button type="submit" class="btn btn-primary action-btn">Sauvegarder</button>
                                                    <a href="<?= APP_URL ?>/index.php?url=admin/evenement-ressources&evenement_id=<?= (int) $selectedEvenementId ?>" class="btn btn-outline action-btn">Annuler</a>
                                                </form>
                                            <?php else: ?>
                                                ...
                                                <div class="ressource-actions">
                                                    ...
                                                    <form action="<?= APP_URL ?>/index.php?url=admin/delete-evenement-ressource/<?= (int) $item['id'] ?>" method="POST" style="display: inline-block;">
                                                        <input type="hidden" name="evenement_id" value="<?= (int) $selectedEvenementId ?>">
                                                        <button type="submit" class="btn action-btn danger" data-confirm="Delete this materiel?">Delete</button>
                                                    </form>
                                                </div>
                                            <?php endif; ?>
                                        </li>
                                    <?php endforeach; ?>
                                <?php else: ?>
                                    <li class="empty">No materiel added yet.</li>
                                <?php endif; ?>
                            </ul>
                        </div>
                        ...
                        <div class="ressource-list-card">
                            ...
                            <ul class="ressource-list">
                                <?php if (!empty($plans)): ?>
                                        ...
                                        <li>
                                            <?php if ($editResource && (int) $editResource['id'] === (int) $item['id']): ?>
                                                <form action="<?= APP_URL ?>/index.php?url=admin/update-evenement-ressource/<?= (int) $item['id'] ?>" method="POST">
                                                    ...
                                                    <div class="form-group" style="margin-bottom: 8px;">
                                                        <textarea name="details" placeholder="Plan details..."><?= htmlspecialchars($item['details'] ?? '') ?></textarea>
                                                    </div>
                                                    <button type="submit" class="btn btn-primary action-btn">Sauvegarder</button>
                                                    <a href="<?= APP_URL ?>/index.php?url=admin/evenement-ressources&evenement_id=<?= (int) $selectedEvenementId ?>" class="btn btn-outline action-btn">Annuler</a>
                                                </form>
                                            <?php else: ?>
                                                ...
                                                <div class="ressource-actions">
                                                    ...
                                                    <form action="<?= APP_URL ?>/index.php?url=admin/delete-evenement-ressource/<?= (int) $item['id'] ?>" method="POST" style="display: inline-block;">
                                                        <input type="hidden" name="evenement_id" value="<?= (int) $selectedEvenementId ?>">
                                                        <button type="submit" class="btn action-btn danger" data-confirm="Delete this plan?">Delete</button>
                                                    </form>
                                                </div>
                                            <?php endif; ?>
                                        </li>
                                    <?php endforeach; ?>
                                <?php else: ?>
                                    <li class="empty">No plan added yet.</li>
                                <?php endif; ?>
                            </ul>
                        </div>
                    </div>
                ...
                <div class="ressource-save-all">
                    <button type="button" id="saveAllResourcesBtn" class="btn btn-yellow">Sauvegarder Tout</button>
                </div>
    </div>
</div>
...
<script>
(function () {
    const saveBtn = document.getElementById('saveAllResourcesBtn');
    if (!saveBtn) {
        return;
    }
    ...
    saveBtn.addEventListener('click', async function () {
        ...
        if (validForms.length === 0) {
            if (existingRightListItems.length > 0) {
                ...
                window.location.href = '<?= APP_URL ?>/index.php?url=admin/evenements';
                return;
            }
