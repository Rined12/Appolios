...
if ($formGlobal) {
    $action = APP_URL . '/index.php?url=admin/chapitres/store-global';
} elseif ($isEdit) {
    $action = APP_URL . '/index.php?url=admin/chapitre/' . (int) $chapter['id'] . '/update';
} else {
    $action = APP_URL . '/index.php?url=admin/course/' . (int) $course['id'] . '/chapitres/store';
}
$backHref = $formGlobal || $isEdit
    ? APP_URL . '/index.php?url=admin/chapitres'
    : APP_URL . '/index.php?url=admin/course/' . (int) $course['id'] . '/chapitres';
?>
<div class="dashboard">
                            ...
                            <label>Cours<br>
                                <select name="course_id" class="form-control" style="max-width:100%;" required>
                                    ...
                                    <?php foreach ($allCourses as $c): ?>
                                        <option value="<?= (int) $c['id'] ?>"><?= htmlspecialchars($c['title']) ?> (<?= htmlspecialchars($c['creator_name'] ?? '') ?>)</option>
                                    <?php endforeach; ?>
                                </select>
                            </label>
                            <br><br>
