        ...
        <div class="admin-layout">
            ...
            <div class="admin-main">
                <div class="dashboard-header" style="display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:12px;">
                    <div>
                        <a href="<?= APP_URL ?>/index.php?url=admin/chapitres" style="color:var(--secondary-color);">← Tous les chapitres</a>
                        <h1 style="margin-top:8px;"><?= htmlspecialchars($course['title']) ?></h1>
                    </div>
                    <a href="<?= APP_URL ?>/index.php?url=admin/course/<?= (int) $course['id'] ?>/chapitres/add" class="btn btn-yellow">Ajouter un chapitre</a>
                </div>
                <?php if (!empty($flash)): ?>
                                ...
                                <?php foreach ($chapters as $ch): ?>
                                    <li style="padding:12px 0;border-bottom:1px solid var(--gray);display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:wrap;">
                                        ...
                                        <div style="display:flex;gap:8px;">
                                            <a href="<?= APP_URL ?>/index.php?url=admin/chapitre/<?= (int) $ch['id'] ?>/edit" class="btn btn-secondary" style="padding:6px 12px;">Modifier</a>
                                            <a href="<?= APP_URL ?>/index.php?url=admin/chapitre/<?= (int) $ch['id'] ?>/delete" class="btn action-btn danger" style="padding:6px 12px;" data-confirm="Supprimer ?">Supprimer</a>
                                        </div>
                                    </li>
                                <?php endforeach; ?>
                            </ul>
