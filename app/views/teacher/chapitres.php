            ...
            <div class="admin-main">
                <div class="dashboard-header" style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:12px;">
                    ...
                    <div style="display:flex;gap:10px;flex-wrap:wrap;">
                        <a href="<?= APP_URL ?>/index.php?url=teacher/chapitres/add" class="btn btn-yellow">Nouveau chapitre</a>
                        <a href="<?= APP_URL ?>/index.php?url=teacher/courses" class="btn btn-outline">Retour aux cours</a>
                    </div>
                </div>
                <?php if (!empty($flash)): ?>
                        ...
                        <table class="data-table" style="width:100%;">
                            ...
                            <?php if (!empty($chapters)): foreach ($chapters as $ch): ?>
                                <tr>
                                    ...
                                    <td>
                                        <a href="<?= APP_URL ?>/index.php?url=teacher/course/<?= (int) ($ch['course_id'] ?? 0) ?>/chapitres" class="btn btn-outline" style="padding:6px 10px;font-size:0.8rem;">Cours</a>
                                        <a href="<?= APP_URL ?>/index.php?url=teacher/chapitre/<?= (int) $ch['id'] ?>/edit" class="btn btn-secondary" style="padding:6px 10px;font-size:0.8rem;">Modifier</a>
                                        <a href="<?= APP_URL ?>/index.php?url=teacher/chapitre/<?= (int) $ch['id'] ?>/delete" class="btn action-btn danger" style="padding:6px 10px;font-size:0.8rem;" data-confirm="Supprimer ce chapitre ?">Supprimer</a>
                                    </td>
                                </tr>
                            <?php endforeach; else: ?>
