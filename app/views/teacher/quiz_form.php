...
$isEdit = !empty($quiz);
$action = $isEdit
    ? APP_URL . '/index.php?url=teacher/quiz/update/' . (int) $quiz['id']
    : APP_URL . '/index.php?url=teacher/quiz/store';
    ...
    <div class="container admin-dashboard-container">
        <div class="admin-layout">
            ...
            <div class="admin-main">
                ...
                <p><a href="<?= APP_URL ?>/index.php?url=teacher/quiz">← Liste</a></p>
                <?php if (!empty($flash)): ?>
                ...
                <form method="post" action="<?= $action ?>" id="quiz-form" class="table-container" style="padding:24px;">
                    <label>Titre du quiz<br><input type="text" name="title" required class="form-control" style="width:100%;max-width:480px;" value="<?= htmlspecialchars($quiz['title'] ?? '') ?>"></label>
                    <br><br>
                    <label>Chapitre<br>
                        <select name="chapter_id" required class="form-control" style="max-width:480px;">
                    ...
                    <br><br>
                    <label>Tags (optionnel)<br><input type="text" name="tags" class="form-control" style="max-width:480px;" value="<?= htmlspecialchars($quiz['tags'] ?? '') ?>"></label>
                    <br><br>
                    <label>Temps limite (secondes, optionnel)<br><input type="number" name="time_limit_sec" class="form-control" style="max-width:200px;" value="<?= htmlspecialchars((string)($quiz['time_limit_sec'] ?? '')) ?>"></label>
                    <hr style="margin:24px 0;">
                    <h3>Importer depuis la banque de questions</h3>
                    <p style="font-size:0.9rem;color:var(--gray-dark);max-width:640px;">Cochez des questions déjà créées dans <a href="<?= APP_URL ?>/index.php?url=teacher/questions">Question</a> : elles seront ajoutées au quiz après les questions saisies ci-dessous (même ordre que les coches).</p>
                    ...
                    <p style="color:var(--gray-dark);font-size:0.9rem;">Au moins 2 options par question. Cochez la bonne réponse.</p>
                    <div id="questions-wrap">
                        <?php foreach ($questions as $qi => $q): ?>
                            <div class="q-block" style="border:1px solid var(--gray);border-radius:8px;padding:16px;margin-bottom:16px;">
                                ...
                                <?php endforeach; ?>
                                <div style="margin-top:8px;">
                                    <label>Index bonne réponse (0 = 1re option)<br>
