...
// ===== APPOLIOS: Theme Toggle (dark-first) =====
function toggleTheme() {
    var isLight = document.body.classList.toggle('tm-light');
...
// ===== APPOLIOS: Courses Table/Grid Toggle, Search, Bulk Select =====
function toggleViewMode(mode) {
    var tableView = document.getElementById('courseTableView');
    var gridView = document.getElementById('courseGridView');
    ...
    localStorage.setItem('coursesView', mode);
}
function filterCourses() {
    var q = document.getElementById('courseSearch').value.toLowerCase();
    var status = document.getElementById('filterStatus').value;
    var rows = document.querySelectorAll('#courseTableView tbody tr, #courseGridView .tm-grid-card');
    rows.forEach(function(row) {
        var title = row.querySelector('.tm-course-name, .tm-grid-title, .tm-course-name');
        var statusCell = row.querySelector('.tm-status, .tm-status-published, .tm-status-draft');
        var matchTitle = !q || (title && title.textContent.toLowerCase().includes(q));
        var matchStatus = !status || (statusCell && statusCell.textContent.toLowerCase().includes(status));
        row.style.display = (matchTitle && matchStatus) ? '' : 'none';
    });
}
function toggleSelectAll() {
    var checked = document.getElementById('selectAllCourses').checked;
    var checkboxes = document.querySelectorAll('.course-checkbox');
    checkboxes.forEach(function(cb) { cb.checked = checked; });
    updateBulkBar();
}
function toggleCourseSelect(cb) {
    updateBulkBar();
}
...
function updateCoursePreview() {
    ...
    if (previewTitle) previewTitle.textContent = title ? title.value : 'Course Title';
    if (previewDesc) previewDesc.textContent = desc ? desc.value : 'Course description...';
    if (previewStatus) {
        previewStatus.textContent = (status && status.value === 'published') ? 'Published' : 'Draft';
        previewStatus.className = 'tm-status ' + ((status && status.value === 'published') ? 'tm-status-published' : 'tm-status-draft');
    }
}
function updateEventPreview() {
    var title = document.getElementById('title');
    var type = document.getElementById('type');
    var date = document.getElementById('date_debut');
    var time = document.getElementById('heure_debut');
    var max = document.getElementById('capacite_max');
    var fields = [
        { id: 'prevTitle', val: title, def: '—' },
        { id: 'prevType', val: type, def: '—' },
        { id: 'prevDate', val: date, def: '—' },
        { id: 'prevTime', val: time, def: '—' },
        { id: 'prevCap', val: max, def: 'Unlimited', suffix: ' people' }
    ];
    fields.forEach(function(f) {
        var el = document.getElementById(f.id);
        if (el) el.textContent = (f.val && f.val.value) ? (f.val.value + (f.suffix || '')) : f.def;
    });
}